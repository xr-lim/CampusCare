...
$app->put('/admin/requests/{id}/assign', function (Request $request, Response $response, array $args) use ($pdo) {
    ...
    $updateStmt = $pdo->prepare(
        'UPDATE maintenance_requests
         SET assigned_technician_id = ?, updated_at = CURRENT_TIMESTAMP
         WHERE id = ?'
    );
...
$app->put('/admin/requests/{id}/status', function (Request $request, Response $response, array $args) use ($pdo) {
    $requestId = adminValidateInteger($args['id'] ?? null);
    ...
    $currentRequest = adminFindRequestSummary($pdo, $requestId);
    if (!$currentRequest) {
        return adminJsonResponse($response, ['message' => 'Maintenance request not found'], 404);
    }
    ...
    if (!in_array($newStatus, adminAllowedStatuses(), true)) {
        return adminJsonResponse($response, ['message' => 'A valid status is required'], 400);
    }

    if ($newStatus === $currentRequest['status']) {
        return adminJsonResponse($response, ['message' => 'Request already has this status'], 400);
    }

    if (strlen($remarks) > 1000) {
        return adminJsonResponse($response, ['message' => 'Remarks must be 1000 characters or fewer'], 400);
    }
    ...
    try {
        ...
        $historyStmt = $pdo->prepare(
            'INSERT INTO status_updates (request_id, updated_by, old_status, new_status, remarks)
             VALUES (?, ?, ?, ?, ?)'
        );
        $historyStmt->execute([
            $requestId,
        ...
        return adminJsonResponse($response, ['message' => 'Unable to update request status'], 500);
    }
